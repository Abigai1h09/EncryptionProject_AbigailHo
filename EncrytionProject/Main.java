class Main {
  ...
  String decoded1 = morseDecrypt(
     asciiDecode(
     brailleDecode(encoded3)
      )
    );

    Input.writeFile("Decode1.txt", decoded1); 
  ...
  String asciiEncode(String txt){
    ...
    return build.toString(); //takes everything to go back to ascii//
  }
  ...
  String asciiDecode(String txt){
    ...
    for(String p : parts){
      if(!p.isEmpty()) // trying to not skip strings
...
String brailleEncode(String txt){
  StringBuilder build = new StringBuilder();
    for(int i = 0; i < txt.length(); i++){
      ...
      if(c == ' ' || c == '\n'){
        build.append(c); // preserve delimiters
      } else {
